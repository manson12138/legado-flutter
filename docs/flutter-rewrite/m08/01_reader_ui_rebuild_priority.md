...
- 离线下载使用 `download_tasks` 表（项目当前总 schemaVersion 11）、`DownloadTaskDao`/`DownloadGateway`/
  `DownloadRepository` 和 App 级 `DownloadCoordinator`；全局并发可持久化设置为 1～8、默认 5，真实请求至少间隔 1.5 秒、
...
- Android 差距复核后的第一阶段已进一步改为真实二维触点、锁定方向与页角、X/Y 同步收尾、
  控制点连线反射和四组方向性阴影；第二阶段已接入受控前后章快照、相邻首页/末页分页、
  章首章末同一次手势及无重复补播提交，等待真机验证；
...
- 设置面板“系统”页新增左右两个独立全局开关，左侧默认开启、右侧默认关闭；旧总开关
  仅兼容迁移到左侧；
- 普通阅读菜单显示时仍允许未被控件占用的边缘退出，业务 Sheet 活跃时禁用；关闭流程
  增加单飞保护，避免与系统返回重复关闭；
...
- TTS、AI、同步等依赖型能力未实现前不展示假入口，或展示明确“待迁移”说明；离线下载已经接入真实入口和协调器，不再属于占位能力。
...
- 点击区域和音量键翻页配置已接入，但仍需真机确认 Android/iOS 是否都能收到系统音量键事件；原生文本选择、复制/分享、书签、高亮和下划线已接入待双端验证，双击、鼠标滚轮和更复杂自定义动作尚未完成；
...
- EPUB 本地图片与 Nav/NCX、MOBI/AZW 家族、压缩包导入、PDF outline/密码交互，以及 AI 总结/清理/改写；TXT/EPUB/UMD 文本阅读和 PDF 基础页面阅读已经接入。
