...
>
> 最后静态核对：2026-07-30。未运行编译、测试、分析、格式化或应用启动。
...
| 本地书导入、Android 外部 TXT“打开方式”关联、导入后阅读与本地书可见入口 | `ui/local_book_import/`、`platform/local_book_platform_bridge.dart`、`platform/external_local_book_open_service.dart`、Android `MainActivity.kt` / `ExternalTxtOpenBridge.kt`；`ui/settings/` 的“本地书籍”通过 `ui/home/welcome_route.dart` 通知保活 `BookshelfRoute` 选择 `BookGroup.idLocal` | `LocalBookImportCoordinator`、`LocalBookStorage`、各格式 Parser；外部 `ACTION_VIEW` 先在后台流式取得有界 cache 临时副本，冷/热启动均进入现有导入确认页；单本成功携带持久书籍快照直接替换到统一阅读入口，批量保留汇总；本地书实际位于当前用户书架并由可见“本地”系统分组过滤，代码等待用户验证 | `model/localBook/`、`FileAssociationActivity` 和原文件导入入口 | [`m08_1/README.md`](./m08_1/README.md)、[`m08_1/android_txt_external_open_association_design.md`](./m08_1/android_txt_external_open_association_design.md)、[`m08_1/local_book_visibility_and_post_import_reader_design.md`](./m08_1/local_book_visibility_and_post_import_reader_design.md) |
...
| 本地封面选择 | `platform/local_book_cover_service.dart`、`ui/change_book_cover/` | `file_picker` 图片选择器；立即复制到 Application Support 私有目录 | `file_picker` / Document Picker；立即复制到 Application Support 私有目录 |
| 本地书文件选择与外部 TXT 打开 | `platform/local_book_platform_bridge.dart`、`platform/external_local_book_open_service.dart` | `file_picker` / SAF；`AndroidManifest.xml` + `MainActivity.kt` + `ExternalTxtOpenBridge.kt` 接收 TXT `ACTION_VIEW`，后台生成有界临时副本并支持一次性清理 | `file_picker` / Document Picker；本轮未声明 iOS 外部打开关联 |
...
| 书架长按单选与书籍详情统一封面替换代码已写入；先复用已有候选，没有候选时搜索当前可见且已启用书源；只接受非空封面，并严格按“书名和作者精确 → 书名精确 → 书名模糊”选择当前最高层级；搜索不到时可从双端系统选择器复制本地图片到应用支持目录；两种来源都只字段级更新 `customCoverUrl`，等待用户运行验收 | [`m11/book_cover_source_search_and_replacement_design.md`](./m11/book_cover_source_search_and_replacement_design.md) |
