...
| 中文 / 英文品牌名 | `拾页` / `PageNest`；Android 使用 `values` 资源，iOS 使用 `InfoPlist.strings` 按系统语言展示 |
| 工程产品名 | Flutter 包名为 `pagenest`；Android Gradle 根项目和 iOS `.app` 归档产品均为 `PageNest`；iOS 保留 Flutter/CocoaPods 约定的 `Runner` Target、Scheme 与宿主目录名 |
| Android applicationId | `com.contradiction.pagenest` |
| Android minSdk | `26` |
| iOS Bundle Identifier | `com.contradiction.pagenest` |
| iOS Deployment Target | `16.0` |
| 独立数据库 | `pagenest.db`，当前 Schema v11；书架、分组、目录、阅读历史、进度、下载附属状态和目录更新检查点按游客或登录账号作用域隔离 |
| 品牌与签名资源 | `assets/branding/pagenest_app_icon_master.png`是双端不透明图标母版；Android 图标在 `res/mipmap-*`，iOS 图标在 `Assets.xcassets/AppIcon.appiconset`；中英文启动名称分别位于 Android `values*/strings.xml` 和 iOS `*/InfoPlist.strings`；Android 私密签名文件仅在本机且被 Git 忽略 |
...
| 根 Widget | `lib/src/app/pagenest_app.dart` | `PageNestBootstrapApp` 先展示轻量启动壳，首帧后单飞初始化 MMKV，失败时降级为进程内偏好，再创建完整依赖；`PageNestApp` 负责主题、初始路由、路由观察器、错误边界、会话恢复和游客/账号本地作用域切换。未登录直接进入游客主界面；当前作用域异步恢复下载并启动书架/历史目录后台更新，已登录普通账号另行启动准入轮询（硬编码 `admin` 账号跳过） |
...
| 登录/注册密码 RSA-OAEP-SHA256 加密 | `platform/password_encryption_platform_service.dart` | `MainActivity.kt` 的 `com.contradiction.pagenest/password_encryption` 通道 | `AppDelegate.swift` 的同名通道和 `SecKey` | [`m11/backend_api_integration/04_authentication_rsa_oaep_upgrade_design.md`](./m11/backend_api_integration/04_authentication_rsa_oaep_upgrade_design.md) |
