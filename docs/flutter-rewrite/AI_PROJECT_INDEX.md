...
全局体验优化机会、静态 UI 不一致、恢复路径、后台任务可见性、弱网/离线、可访问性与建议执行批次见
[`UX_OPTIMIZATION_OPPORTUNITY_ANALYSIS.md`](./UX_OPTIMIZATION_OPPORTUNITY_ANALYSIS.md)；当前为
`PROPOSED`，建议先执行不改 Schema 的“体验收口包”，再按用户确认进入搜索候选持久化、一键换源、
任务中心、TTS 或宽屏等专项。
...
搜索结果进入详情时复用 `SearchBook` 基础字段渐进渲染、缺失字段异步补齐且不闪屏，以及详情/阅读期间
暂停多书源搜索、最终返回搜索页后从原进度继续的静态分析与实施边界见
[`m06/03_search_detail_progressive_loading_and_pause_resume_design.md`](./m06/03_search_detail_progressive_loading_and_pause_resume_design.md)；
当前为 `IMPLEMENTED`：详情快照渐进渲染、封面旧帧保持、活动书源可恢复暂停，以及基于稳定
`RouteObserver` 的最终返回恢复均已写入，等待用户运行验证。
搜索结果 Cell 当时聚合的完整书源候选只由搜索入口整组覆盖 SQLite，书架、阅读器、下载管理和其他详情
...
| 应用组合根 | `lib/src/app/app_dependencies.dart`、`app/current_user_scope.dart`、`app/reader_intro_preferences.dart`、`app/bookshelf_history_startup_preloader.dart`、`app/bookshelf_history_auto_refresh_service.dart`、`app/guest_book_source_import_service.dart` | DAO、Repository、HTTP、JS、协调器和 UseCase 的唯一集中装配处；首帧后先取得 MMKV 或内存降级的 `AppPreferencesStore`，只向业务注入类型化服务；首次阅读先导页按用户与书籍摘要隔离；游客使用本机保留 ID `-1`，登录账号使用服务端 ID，二者共用无 Token 的本地作用域；启动时单飞预读书架、分组和历史首快照，并在下载恢复后以两个 worker 按 URL 去重后台更新目录；游客 URL/邀请码书源导入只在当前操作内短暂持有凭证和游标 |
| 路由常量 | `lib/src/app/app_route.dart` | 应用内稳定路由名 |
| 路由与参数校验 | `lib/src/app/app_router.dart` | Route 创建、构造注入、无效参数错误页 |
| 根 Widget | `lib/src/app/legado_app.dart` | `LegadoBootstrapApp` 先展示轻量启动壳，首帧后单飞初始化 MMKV，失败时降级为进程内偏好，再创建完整依赖；`LegadoApp` 负责主题、初始路由、路由观察器、错误边界、会话恢复和游客/账号本地作用域切换。未登录直接进入游客主界面；当前作用域异步恢复下载并启动书架/历史目录后台更新，已登录普通账号另行启动准入轮询（硬编码 `admin` 账号跳过） |
...
| `/book-sources` | `ui/book_source/book_source_route.dart` / `book_source_screen.dart` | `BookSourceManagementViewModel` | 书源管理、导入、扫码和编辑；登录账号继续使用 API v2 游标同步；游客“更多 → 输入 URL”支持远程 JSON 或管理员邀请码的内存态游客分页同步，代码已写入等待用户验证，方案见 [`m11/guest_book_source_url_and_invitation_import_design.md`](./m11/guest_book_source_url_and_invitation_import_design.md)；同步或添加成功后以“取消/确定”引导进入搜索页的代码已写入，实施记录见 [`m11/book_source_completion_search_navigation_design.md`](./m11/book_source_completion_search_navigation_design.md)，等待用户验证 |
...
| `/bookshelf` | `ui/bookshelf/bookshelf_route.dart` / `bookshelf_screen.dart` / `reading_history_screen.dart` / `book_grid_layout.dart` / `app/bookshelf_layout_preferences.dart` | `BookshelfViewModel`、`ReadingHistoryViewModel` | 内部书架/历史/本地三页 `PageView`；本地书只在本地页展示，书架和历史排除本地书；三页只在存在漫画时显示“书籍/漫画”分类，本地空页提供默认封面导入入口；本地长按管理不提供联网刷新或换源；书架、历史与本地共用封面网格规格，列表/网格排版通过 MMKV 持久化并在重启后恢复 |
...
| 普通规则 | `model/analyze_rule/standard_rule_engine.dart` | `source_rules.dart`、`standard_source_parser.dart`、`standard_source_service.dart`；JSONPath 支持 Android `AnalyzeByJSonPath.innerRule` 同款的平衡 `{$...}` 文本/URL 内嵌求值 | `model/analyzeRule/`、`model/webBook/` | [`m03/README.md`](./m03/README.md) |
...
| 搜索 | `ui/search/`、`app/search_preferences.dart` | `BookSearchCoordinator`、`SearchHistoryRepository`、`SearchPreferences`、`StandardBookSourceService`；书名/作者/其他分类栏由现有 `PageController.page` 驱动单块共享背景与文字颜色插值；搜索书源下方提供等宽“书籍/漫画”整类开关，自定义书源保存后两个快捷开关都取消高亮；结果 cell 首次封面地址固定，新增同组来源不再触发封面替换 | `ui/book/search/` | 搜索结果匹配方式继续使用设备级 `caches`；搜索关键字历史已按登录用户派生缓存键隔离，旧公共键首次访问删除；保活 `SearchViewModel` 直接监听用户作用域，在认证恢复、登录、退出或换号时清空旧瞬时搜索状态、重读新作用域历史并拒绝旧异步结果，等待用户验收；云端同步、导入和启停书源后，保活搜索页通过 `BookSourceGateway.watchAll()` 实时更新下一次搜索使用的启用书源，实施记录见 [`m11/remote_book_source_search_live_refresh_analysis.md`](./m11/remote_book_source_search_live_refresh_analysis.md)；无书源、全局全停用和搜索页零选择的判定优先级、跨一级页面导航及一次性提示代码已写入，等待用户验证，见 [`m06/05_search_source_empty_state_navigation_design.md`](./m06/05_search_source_empty_state_navigation_design.md)；[`m06/README.md`](./m06/README.md)、[`m06/02_search_result_and_book_info_interaction_plan.md`](./m06/02_search_result_and_book_info_interaction_plan.md)、[`m11/search_selector_and_reader_detail_entry_design.md`](./m11/search_selector_and_reader_detail_entry_design.md)、[`m11/user_scoped_search_history_design.md`](./m11/user_scoped_search_history_design.md) |
| 详情和目录 | `ui/book_info/`、`ui/change_book_cover/`、`platform/local_book_cover_service.dart`、`ui/components/book_cover.dart`、`model/web_book/book_cover_search_coordinator.dart`、`domain/gateway/book_cover_candidate_gateway.dart`、`data/repository/book_cover_candidate_repository.dart`、`domain/usecase/update_book_custom_cover_use_case.dart` | `BookDetailService`、`SaveBookChaptersUseCase`、`AddBookToBookshelfUseCase`；详情主操作区“更新目录”复用显式刷新详情与完整目录链路；详情操作卡、菜单和封面长按打开共用封面面板；封面面板可搜索书源候选或通过系统选择器复制本地图片到应用私有目录，本地图片持久化为 `legado-cover` 稳定标识并兼容旧绝对路径；已入架网络书“替换书源”已与未入架已有候选切换分离，始终进入重新搜索整书换源页 | `ui/book/info/`、`ui/book/changecover/`、`ui/book/info/edit/BookInfoEditScreen.kt`、`BookInfoEditViewModel.coverChangeTo` | [`m06/README.md`](./m06/README.md)、[`m06/01_book_info_ui_rebuild_priority.md`](./m06/01_book_info_ui_rebuild_priority.md)、[`m06/02_search_source_ranking_and_detail_failover_plan.md`](./m06/02_search_source_ranking_and_detail_failover_plan.md)、[`m11/book_cover_source_search_and_replacement_design.md`](./m11/book_cover_source_search_and_replacement_design.md)、[`m11/change_source/04_detail_reader_entry_and_fresh_search_plan.md`](./m11/change_source/04_detail_reader_entry_and_fresh_search_plan.md) |
| 书架 | `ui/bookshelf/`、`ui/change_book_cover/`、`platform/local_book_cover_service.dart`、`ui/components/book_cover.dart`、`data/dao/book_dao.dart`、`data/dao/reading_history_dao.dart`、`domain/usecase/update_book_custom_cover_use_case.dart` | `BookRepository`、`BookGroupRepository`、`BookshelfRefreshCoordinator`；`app/bookshelf_history_auto_refresh_service.dart` 在登录启动后后台更新可更新网络书，并通过 `TocRefreshCheckpointDao` 实现 30 分钟节流、目录尾部增量更新、异常全量回退和 7 天完整校准；长按选择态保留搜索/分类/排序区避免内容上跳，底部悬浮操作条以图标和文字展示全部动作，列表和网格强化选中边框；长按单选可打开封面面板，网络候选和本地图片都字段级更新 `customCoverUrl`，并在同一事务同步已有阅读历史快照 | `ui/main/bookshelf/`、`ui/book/changecover/`、`ui/book/info/edit/BookInfoEditScreen.kt` | [`m07/README.md`](./m07/README.md)、[`m11/bookshelf_history_startup_auto_toc_refresh_design.md`](./m11/bookshelf_history_startup_auto_toc_refresh_design.md)、[`m11/same_title_bookshelf_conflict_and_merge_design.md`](./m11/same_title_bookshelf_conflict_and_merge_design.md)、[`m11/book_cover_source_search_and_replacement_design.md`](./m11/book_cover_source_search_and_replacement_design.md) |
| 书架内书架/历史/本地三页 | `ui/bookshelf/bookshelf_page_switcher.dart`、`bookshelf_contract.dart`、`bookshelf_view_model.dart`、`reading_history_contract.dart`、`reading_history_view_model.dart`、`reading_history_screen.dart`；`ui/reader/` 写入历史并提供显式加入书架图标 | `ReadingHistoryDao` / `ReadingHistoryRepository` / `ReadingHistoryGateway` / `RecordReadingHistoryUseCase` / `DeleteReadingHistoryUseCase`；历史列表和网格支持长按多选、全选及确认删除；本地记录仍复用现有本地关系数据，但 UI 只在本地页展示，不进入书架或历史页 | Android `ReadRecord`、本地书与书架/阅读入口 | [`m11/reading_history_and_bookshelf_pageview_design.md`](./m11/reading_history_and_bookshelf_pageview_design.md)、[`m11/bookshelf_history_fixed_header_interaction_plan.md`](./m11/bookshelf_history_fixed_header_interaction_plan.md)、[`m08_1/local_book_visibility_and_post_import_reader_design.md`](./m08_1/local_book_visibility_and_post_import_reader_design.md) |
| 按游客/登录账号隔离本地书架、分组、目录、阅读历史、进度和下载附属状态；`app/current_user_scope.dart` 使用 `-1` 作为仅限本机的游客作用域，账号继续使用服务端 ID，v8 到 v9 破坏式升级与作用域切换失效规则 | `data/local/legado_database.dart`、`data/local/database_change_notifier.dart`、`data/dao/`、`data/repository/`、`app/current_user_scope.dart`、`app/bookshelf_history_startup_preloader.dart`、`app/legado_app.dart`、`ui/bookshelf/bookshelf_view_model.dart`、`ui/bookshelf/reading_history_view_model.dart`、`model/reader/download_coordinator.dart` | 游客与同设备多账号隔离代码已写入，等待用户验证；保活书架 Route 的两个 ViewModel 直接监听作用域并重建数据库流，三个列表 DAO 通过可立即取消且不会遗漏提交的 `watchQuery()` 避免认证恢复后 loading 等待下一次表写入；游客数据不自动迁移到账号，当前无服务端书架/历史同步 API，不支持跨设备恢复 | [`m11/user_scoped_bookshelf_and_history_design.md`](./m11/user_scoped_bookshelf_and_history_design.md)、[`m11/optional_authentication_guest_mode_design.md`](./m11/optional_authentication_guest_mode_design.md) |
| App 可选登录、游客本地作用域与账号管理；关于页版本检查和平台更新入口 | `main.dart`、`platform/app_package_info_service.dart`、`platform/android_apk_update_service.dart`、`api/remote_app/remote_app_service_config.dart`、`app/current_user_scope.dart`、`app/legado_app.dart`、`ui/authentication/`、`ui/settings/`、`ui/about/`、`domain/gateway/authentication_gateway.dart`、`data/repository/authentication_repository.dart` | 安全会话恢复、`userId=-1` 游客数据、RSA-OAEP 密码传输、权限读取与退出登录；启动配置显式传 `android|ios`，Android 服务端 APK 经 Range 续传、大小/SHA-256 校验和系统安装器更新，iOS 屏蔽 APK 并只前往 TestFlight；登录/退出通过根 key 重建隔离游客与账号页面及后台任务；等待双端用户验收 | Android `AppUpdateBridge.kt`、FileProvider 与 App 登录/注册相关入口 | [`m11/backend_api_integration/03_app_access_and_update_plan.md`](./m11/backend_api_integration/03_app_access_and_update_plan.md)、[`m11/optional_authentication_guest_mode_design.md`](./m11/optional_authentication_guest_mode_design.md)、[`m11/backend_api_integration/10_authentication_entry_and_full_flow_plan.md`](./m11/backend_api_integration/10_authentication_entry_and_full_flow_plan.md)、[`m11/authentication_startup_splash_and_input_readiness_plan.md`](./m11/authentication_startup_splash_and_input_readiness_plan.md)、[`m11/backend_api_integration/11_persistent_app_access_and_update_state_design.md`](./m11/backend_api_integration/11_persistent_app_access_and_update_state_design.md)、[`m11/backend_api_integration/13_registration_validation_and_error_feedback_analysis.md`](./m11/backend_api_integration/13_registration_validation_and_error_feedback_analysis.md) |
...
| 漫画搜索、入架与阅读 | `/manga-reader` 的 `ui/manga_reader/` 为真实 MVI 阅读器；`model/manga/read_manga_coordinator.dart` 连接书籍/目录、正文缓存、书源、三章窗口和固定 worker 预取，`domain/model/manga_reader_config.dart` 对齐 Android 阅读模式，`domain/model/manga_reading_position.dart` 隔离章节/图片语义与 `chapterPos` 持久适配，`manga_page_view.dart` 惰性消费 `MangaImageGateway` 本地文件并负责当前页缩放；类型、解析、网络和缓存入口分别位于 `constant/book_type.dart` / `book_source_type.dart`、`model/manga/manga_content_parser.dart`、`domain/gateway/manga_image_gateway.dart`、`data/repository/manga_image_repository.dart`、`data/cache/manga_image_cache.dart` | 步骤 0 已通过，步骤 1～10 已写入并统一待最终验证；支持详情/书架/历史恢复、条漫深页锚点、三种单页、缩放、范围收敛和串行进度、账号/前后台/内存压力生命周期、三章解析窗口、双 worker 图片预取、64 MiB 解码预算，以及章节重试/刷新、书源登录完成后单次刷新、图片兼容整书换源和新主键路由替换；2026-08-02 真机失败复盘后，图片仓储补齐 URL/Header JavaScript、默认 UA、书源登录 Header、GET/POST 请求选项和 QuickJS `imageDecode` 字节处理，`proxy/dnsIp/serverID` 仍为明确未支持边界；Android/iOS 首版验收尚未执行 | `ui/book/manga/ReadMangaActivity.kt`、`ReadMangaViewModel.kt`、`model/ReadManga.kt`、`model/ImageProvider.kt`、`MangaAdapter.kt`、`MangaLayoutManager.kt`、`help/book/BookHelp.kt#flowImages`、`model/analyzeRule/AnalyzeUrl.kt`、`utils/ImageUtils.kt` | [`m11/manga/README.md`](./m11/manga/README.md)、[`m11/manga/01_sample_and_acceptance_baseline.md`](./m11/manga/01_sample_and_acceptance_baseline.md) |
...
| 本地书导入、Android/iOS 外部 TXT“打开方式”关联、导入后阅读与本地书可见入口 | `ui/local_book_import/`、`platform/local_book_platform_bridge.dart`、`platform/external_local_book_open_service.dart`；Android `MainActivity.kt` / `ExternalTxtOpenBridge.kt`；iOS `Info.plist` / `SceneDelegate.swift` / `AppDelegate.swift` / `ExternalTxtOpenBridge.swift`；`ui/settings/` 的“本地书籍”通过 `ui/home/welcome_route.dart` 通知保活 `BookshelfRoute` 切到本地页 | `LocalBookImportCoordinator`、`LocalBookStorage`、各格式 Parser；Android `ACTION_VIEW` 与 iOS `public.plain-text` Scene URL 都先在后台流式取得有界 cache 临时副本，冷/热启动均进入同一导入确认页；iOS 正式副本保存到 Application Support 下的 `local_books/`，Android 保持既有数据库同级目录；单本成功携带持久书籍快照直接替换到统一阅读入口，批量保留汇总；本地书只在同级“本地”页展示，空页默认封面可直接打开导入，代码等待用户验证 | `model/localBook/`、`FileAssociationActivity` 和原文件导入入口 | [`m08_1/README.md`](./m08_1/README.md)、[`m08_1/android_txt_external_open_association_design.md`](./m08_1/android_txt_external_open_association_design.md)、[`m08_1/local_book_visibility_and_post_import_reader_design.md`](./m08_1/local_book_visibility_and_post_import_reader_design.md) |
...
| 成人内容屏蔽 | `ui/about/`（连点解锁入口） | `AdultContentGateway`/`AdultContentRepository`（内置 Base64 关键词作为最低基线，远程启用词并集扩展，`caches` 保存开关和远程规则；`origin == 'loc_book'` 的用户导入书固定豁免）；`BookSourceRepository` 统一过滤书源管理可见性，`BookSearchCoordinator` 统一过滤书名/作者/分类/简介和搜索/整书换源/单章换源的执行书源；规则修订会刷新保活列表并取消旧搜索快照 | `help/source/SourceHelp.list18Plus`；当前 Android 可追踪基准只覆盖 18+ 域名导入拦截，本 Flutter 关键词可见性策略为独立扩展 | [`m11/adult_content_source_visibility_extension_plan.md`](./m11/adult_content_source_visibility_extension_plan.md)：9 个新增词和书源名称/分组包含即隐藏的代码已写入，等待用户验证 |
...
应用不再打包或自动导入内置书源；新安装书源表保持为空，由用户通过文件、二维码、
剪贴板、远程 URL/邀请码或登录后服务器同步导入，入库继续统一经过
`ImportBookSourcesUseCase` 与 `BookSourceRepository.importSourceJson`。
...
App 内主动导入:
  ...
  -> LocalBookPlatformBridge / file_picker

Android 外部 TXT:
  Android ACTION_VIEW
  -> MainActivity / ExternalTxtOpenBridge
  -> ExternalLocalBookOpenService
  -> LegadoApp 顺序导航
  -> LocalBookImportRoute（预选但不自动导入）

iOS 外部 TXT:
  public.plain-text 文档关联
  -> SceneDelegate 冷/热启动 URL Context
  -> AppDelegate / ExternalTxtOpenBridge
  -> ExternalLocalBookOpenService
  -> LegadoApp 顺序导航
  -> LocalBookImportRoute（预选但不自动导入）
...
BookInfoViewModel
  ...
  -> BookshelfGateway.getShelfBookNameConflict
  -> 精确 URL 未命中后由 BookDao 只按当前用户的 name 查询
  -> 已入架 / 同名冲突 / 未入架
  -> 冲突时由 BookInfoRoute 或 ReaderRoute 展示覆盖、再次添加；返回或点击外部取消
...
|---|---|---|---|
| iOS 插件依赖与构建配置 | `pubspec.yaml` 的项目级 Flutter 配置固定使用 CocoaPods，避免新版 SDK 自动切换依赖管理器 | 不适用 | `ios/Podfile`、`ios/Podfile.lock`、`ios/Flutter/Debug.xcconfig`、`ios/Flutter/Profile.xcconfig`、`ios/Flutter/Release.xcconfig`、`ios/Runner.xcodeproj/project.pbxproj`；Debug/Profile/Release 分别包含对应 Pods 配置；Podfile 的 `post_install` 把所有 Pod Target 收敛到 iOS 16，并把简繁转换插件误入 Compile Sources 的隐私清单移动到 Resources |
...
| 本地封面选择 | `platform/local_book_cover_service.dart`、`ui/change_book_cover/`、`ui/components/book_cover.dart` | `file_picker` 图片选择器；立即复制到 Application Support 私有目录，数据库保存稳定 `legado-cover` 标识 | `file_picker` / Document Picker；立即复制到 Application Support 私有目录，以稳定标识适配可能变化的容器绝对路径 |
| 本地书文件选择与外部 TXT 打开 | `platform/local_book_platform_bridge.dart`、`platform/external_local_book_open_service.dart` | `file_picker` / SAF；`AndroidManifest.xml` + `MainActivity.kt` + `ExternalTxtOpenBridge.kt` 接收 TXT `ACTION_VIEW`，后台生成有界临时副本并支持一次性清理 | `file_picker` / Document Picker；`Info.plist` 声明 `public.plain-text`，`SceneDelegate.swift` + `AppDelegate.swift` + `ExternalTxtOpenBridge.swift` 接收冷/热启动文件 URL、协调安全作用域并生成同契约有界临时副本；等待真机验证 |
...
| 实际安装包版本身份 | `platform/app_package_info_service.dart`、`api/remote_app/remote_app_service_config.dart`（启动时一次性读取，并由组合根提供给“我的”与“关于”页面） | `MainActivity.kt` 通过 `PackageManager` 返回当前包 `versionName` 与 `longVersionCode` | `AppDelegate.swift` 返回 `CFBundleShortVersionString` 与 `CFBundleVersion` |
| App 更新 | `platform/android_apk_update_service.dart`、`app/legado_app.dart`、`api/remote_app/remote_app_api.dart` | `android/app/src/main/kotlin/io/legado/flutter/AppUpdateBridge.kt` 流式 Range 续传、大小/SHA-256 校验、未知来源权限和系统安装器；`AndroidManifest.xml` 与 `res/xml/app_update_file_paths.xml` 只授权安装器读取已校验 APK | 不注册 APK 桥；服务端平台响应的 `downloadUrl` 只作为 TestFlight 外链，UI 隐藏 APK 地址、大小、进度和安装入口 |
...
| 游客书源管理不显示账号云同步按钮；“更多”增加游客专用 URL 输入，HTTP/HTTPS 返回 JSON 复用现有导入确认，非 URL 邀请码兑换内存态游客凭证并按 50 条游标分页导入；登录账号旧逻辑保持不变；代码已写入，等待用户验证；邀请码兑换阶段出现“应用签名或设备时间校验失败”时，优先核对安装包 HMAC 构建配置与部署端配置，再检查设备绝对时间 | [`m11/guest_book_source_url_and_invitation_import_design.md`](./m11/guest_book_source_url_and_invitation_import_design.md)、[`m11/guest_book_source_signature_failure_analysis.md`](./m11/guest_book_source_signature_failure_analysis.md) |
...
| 用户主动加入网络书时只按书名提示覆盖/再次添加；详情与阅读器代码已写入，覆盖使用新活动书源并保留旧进度、分组、备注、自定义封面和阅读配置，新普通封面为空时回退旧封面，等待用户验证 | [`m11/same_title_bookshelf_conflict_and_merge_design.md`](./m11/same_title_bookshelf_conflict_and_merge_design.md) |
| 书架选择态保留搜索/分类/排序区并使用不改变列表高度的底部文字操作条，列表与网格强化选中边框；历史列表和网格支持长按多选、全选及确认删除，历史删除不影响书架且启动迟到刷新不能复活记录；代码等待用户运行验收 | [`m07/README.md`](./m07/README.md)、[`m11/reading_history_and_bookshelf_pageview_design.md`](./m11/reading_history_and_bookshelf_pageview_design.md)、[`m11/bookshelf_history_fixed_header_interaction_plan.md`](./m11/bookshelf_history_fixed_header_interaction_plan.md) |
| 书架长按单选与书籍详情统一封面替换代码已写入；先复用已有候选，没有候选时搜索当前可见且已启用书源；只接受非空封面，并严格按“书名和作者精确 → 书名精确 → 书名模糊”选择当前最高层级；搜索不到时可从双端系统选择器复制本地图片到 Application Support，并仅持久化可跨双端重启解析的 `legado-cover` 稳定标识，兼容旧绝对路径；封面字段在同一事务同步书架和已有阅读历史快照，进程内同步缓存避免来回切页反复闪占位，等待用户运行验收 | [`m11/book_cover_source_search_and_replacement_design.md`](./m11/book_cover_source_search_and_replacement_design.md) |
| 详情页整书换源主入口已不再被已有候选分支截走；阅读器更多菜单已同时提供“整书换源/单章换源”，并补齐未入架校验及取消或异常返回后的系统模式、定时器和焦点恢复；代码等待用户运行验收 | [`m11/change_source/04_detail_reader_entry_and_fresh_search_plan.md`](./m11/change_source/04_detail_reader_entry_and_fresh_search_plan.md) |
