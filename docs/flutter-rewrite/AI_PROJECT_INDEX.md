...
| 搜索 | `ui/search/`、`app/search_preferences.dart` | `BookSearchCoordinator`、`SearchHistoryRepository`、`SearchPreferences`、`StandardBookSourceService`；书名/作者/其他分类栏由现有 `PageController.page` 驱动单块共享背景与文字颜色插值，等待用户验证 | `ui/book/search/` | 搜索结果匹配方式继续使用设备级 `caches`；搜索关键字历史已按登录用户派生缓存键隔离，旧公共键首次访问删除；保活 `SearchViewModel` 直接监听用户作用域，在认证恢复、登录、退出或换号时清空旧瞬时搜索状态、重读新作用域历史并拒绝旧异步结果，等待用户验收；云端同步、导入和启停书源后，保活搜索页通过 `BookSourceGateway.watchAll()` 实时更新下一次搜索使用的启用书源，实施记录见 [`m11/remote_book_source_search_live_refresh_analysis.md`](./m11/remote_book_source_search_live_refresh_analysis.md)；无书源、全局全停用和搜索页零选择的判定优先级、跨一级页面导航及一次性提示代码已写入，等待用户验证，见 [`m06/05_search_source_empty_state_navigation_design.md`](./m06/05_search_source_empty_state_navigation_design.md)；[`m06/README.md`](./m06/README.md)、[`m06/02_search_result_and_book_info_interaction_plan.md`](./m06/02_search_result_and_book_info_interaction_plan.md)、[`m11/search_selector_and_reader_detail_entry_design.md`](./m11/search_selector_and_reader_detail_entry_design.md)、[`m11/user_scoped_search_history_design.md`](./m11/user_scoped_search_history_design.md) |
...
| 本地书导入、Android/iOS 外部 TXT“打开方式”关联、导入后阅读与本地书可见入口 | `ui/local_book_import/`、`platform/local_book_platform_bridge.dart`、`platform/external_local_book_open_service.dart`；Android `MainActivity.kt` / `ExternalTxtOpenBridge.kt`；iOS `Info.plist` / `SceneDelegate.swift` / `AppDelegate.swift` / `ExternalTxtOpenBridge.swift`；`ui/settings/` 的“本地书籍”通过 `ui/home/welcome_route.dart` 通知保活 `BookshelfRoute` 选择 `BookGroup.idLocal` | `LocalBookImportCoordinator`、`LocalBookStorage`、各格式 Parser；Android `ACTION_VIEW` 与 iOS `public.plain-text` Scene URL 都先在后台流式取得有界 cache 临时副本，冷/热启动均进入同一导入确认页；iOS 正式副本保存到 Application Support 下的 `local_books/`，Android 保持既有数据库同级目录；单本成功携带持久书籍快照直接替换到统一阅读入口，批量保留汇总；本地书实际位于当前用户书架并由可见“本地”系统分组过滤，代码等待用户验证 | `model/localBook/`、`FileAssociationActivity` 和原文件导入入口 | [`m08_1/README.md`](./m08_1/README.md)、[`m08_1/android_txt_external_open_association_design.md`](./m08_1/android_txt_external_open_association_design.md)、[`m08_1/local_book_visibility_and_post_import_reader_design.md`](./m08_1/local_book_visibility_and_post_import_reader_design.md) |
...
|---|---|---|---|
| iOS 插件依赖与构建配置 | `pubspec.yaml` 的项目级 Flutter 配置固定使用 CocoaPods，避免新版 SDK 自动切换依赖管理器 | 不适用 | `ios/Podfile`、`ios/Podfile.lock`、`ios/Flutter/Debug.xcconfig`、`ios/Flutter/Profile.xcconfig`、`ios/Flutter/Release.xcconfig`、`ios/Runner.xcodeproj/project.pbxproj`；Debug/Profile/Release 分别包含对应 Pods 配置 |
