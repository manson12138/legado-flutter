...
仍未完成且不得宣称完成的内容：覆盖与仿真翻页代码均等待用户真机验证，淡入等其余独立翻页呈现层、真正同时保留多章 Widget 的有限章节窗口、预取状态可视化、其余低频页面的逐页视觉重构，以及 Android/iOS 完整真机验收。
...
仿真卷页已进入 R6 专项并写入第一批代码，仍不阻塞其他模式验收，真机结果见 M8 专项记录。
