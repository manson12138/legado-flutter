...
- 游客书源管理页的“更多”新增“输入 URL”：HTTP/HTTPS 返回内容继续进入现有 JSON 导入确认；非 URL 短文本按管理员邀请码兑换最长一小时的内存态游客凭证，并通过 `/api/v1/booksource/guest/page` 每批 50 条导入。登录账号的原 V2 云同步入口、权限和持久断点保持不变。
- 游客邀请码、`guestToken`、Authorization、App nonce、签名、URL 和书源响应正文统一使用 `GUEST_BOOK_SOURCE_IMPORT` 摘要日志保护，不写数据库、缓存、安全存储、扫码诊断文件或埋点。
...
4. 独立远程 URL 和游客邀请码入口代码已写入、等待用户验证；导出、拖拽排序、置顶/置底、分组追加/移除和全量高级字段编辑仍未实现。
...
17. 游客状态确认不显示账号云同步按钮，“更多”显示“输入 URL”；输入 HTTP/HTTPS JSON 地址后确认仍进入原覆盖/跳过导入对话框。
18. 游客输入有效管理员邀请码，确认跨至少三批完成同步；无效、过期、普通用户、产品不匹配、管理员禁用和权限撤销邀请码显示固定安全提示。
19. 登录有权限和无权限账号分别确认旧入口、权限和 V2 游标断点行为不变，且“更多”不显示游客入口。
20. 检查数据库、缓存、安全存储、日志、崩溃报告和埋点中不存在邀请码、邀请码哈希或 `guestToken`。
