...
- `useAllSources=false` 时保留仍有效的旧选择，并自动勾选本次新增且启用、可见的书源；
- 不清空关键词、历史和已有搜索结果；
...
4. 搜索页处于部分选择或零选择时新增书源，返回后确认新书源已自动勾选，原有有效选择保持不变。
5. 输入关键词搜索，进度总数和实际调度书源应包含新书源。
6. 在书源页停用或删除一个书源，返回搜索页后该书源应立即从候选和显式选择中移除。
7. 搜索进行中再同步书源：当前搜索不重启、不插入新 worker；下一次搜索使用最新书源。
8. 多次切换搜索页和书源页后，不应出现重复刷新、重复搜索、页面销毁后更新或异常提示。

## 7. 状态判断

搜索协调器的启用书源观察流和搜索 ViewModel 生命周期订阅已经写入。同步、手动导入、编辑、启停或删除书源后，搜索页会基于现有 `book_sources` 表级通知更新候选；新增且启用、可见的书源会自动进入搜索页选择范围，正在运行的搜索仍使用启动时快照。
