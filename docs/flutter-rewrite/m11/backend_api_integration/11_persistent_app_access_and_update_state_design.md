...
状态：`PROPOSED / 待用户确认实施`。本文基于 2026-07-25 的静态梳理；未运行编译、测试、分析、格式化或应用。
...
- 当前客户端身份：`productId`、`appVersionName`、`appVersionCode`、`channel`。
...
当持久化记录的客户端身份与当前安装版本不一致时，必须忽略并删除该记录：新版本不能被旧版本的强制升级或拒绝状态永久阻断。只有服务端成功返回 `allowed=true` 且 `forceUpdate=false` 时，才允许覆盖、解除此前阻断。

为避免缓存读取尚未完成时短暂进入业务界面，协调器增加“正在恢复本地准入状态”的受控启动态，应用级遮罩仅等待这一次本地 SQLite 读取；不等待网络。读取失败时记录不含数据内容的统一日志后按无缓存降级，不阻塞登录页或无限等待。

## 4. 拟修改范围

- `RemoteAppConfigurationRepository`：负责准入/升级状态的 JSON 编解码、按当前客户端身份读取、成功 bootstrap 后写入和身份不匹配清理。
...
5. 安装具有不同 versionName 或 versionCode 的新包后断网启动：旧包缓存不应阻断新包。
