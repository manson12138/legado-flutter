...
状态：`IN_PROGRESS`。`GET /api/v1/app/bootstrap` 的准入和升级状态已接入前台协调器：登录后先恢复与当前安装包身份匹配的本地已确认状态，再立即检查，前台每 5 分钟检查；明确拒绝或强制更新时显示阻断 UI。Android 提供用户主动退出；服务端提供合法 HTTPS `downloadUrl` 时会完整显示地址，点击通过系统外部浏览器打开、长按复制，Android 与 iOS 均适用。等待 Android 后 iOS 用户验收，AI 未运行构建、测试或检查。
...
| 有非强制更新 | 显示可关闭的升级弹窗；完整展示 HTTPS 下载地址，点击外部浏览器打开、长按复制，稍后再说后继续使用 | 同左 |
| 有强制更新 | 显示不可关闭的升级弹窗并阻断业务；完整展示 HTTPS 下载地址，点击外部浏览器打开、长按复制；Android 可退出 | 同左的阻断语义 |
...
在链接未定义前，非强制更新弹窗只能提供“知道了/稍后”，强制更新页只能说明“请从发布渠道获取最新版本”，不得显示无响应的“立即更新”按钮。合法 HTTPS 链接存在时，两个平台均须完整显示该地址，点击通过系统外部浏览器访问，长按复制地址。
