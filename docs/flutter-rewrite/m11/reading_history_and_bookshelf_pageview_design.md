...
5. 书架和历史都支持长按多选；系统返回、切换双页、切换主导航目的地或页面失去可见性时自动取消当前选择。
6. 历史批量删除只移除独立历史书籍和级联目录快照，不删除同书的书架成员、本地原文件或共享阅读数据。

不包含读时统计、同步、历史筛选/搜索、书架分组规则改造或 Android `ReadRecord` 统计迁移。
...
- 新增 `ReadingHistoryDao`、`ReadingHistoryRepository`、
  `ReadingHistoryGateway`、`RecordReadingHistoryUseCase` 和 `DeleteReadingHistoryUseCase`。网络书首次成功得到可读正文后写入
  独立书籍/目录快照；TXT/EPUB 复用同一文本链路，PDF 在文档真实打开后写入；后续保存阅读
  或 PDF 页码进度时同步更新历史位置。
- 书架路由内部改为书架/历史两页 `PageView`。两页选择模式在返回、滑离当前页或主导航离开书架时退出，
...
- 原 `BookshelfViewModel` 继续只管理书架分组、刷新和选择；历史使用独立 `ReadingHistoryViewModel`，实时观察历史数据、按最后阅读时间排序并管理历史选择和删除确认。
- 从当前页滑离、点击另一页签或一级书架目的地被 `IndexedStack` 隐藏时，分别退出书架或历史选择；返回优先消费当前选择模式。
- 历史长按进入多选，列表和网格都展示高对比度边框及勾选状态；顶部直接显示“全选/删除”文字动作。确认删除经 `DeleteReadingHistoryUseCase` 进入单一 SQLite 事务，目录依赖外键级联。
- 启动期目录自动更新写回历史时使用 `refreshExistingHistory` 在同一事务重新确认成员仍存在，关闭“用户删除后迟到后台结果重新创建历史”的竞态；删除成功同时失效启动旧快照。
...
8. 在历史列表和网格分别长按并多选，确认顶部显示选择数量、全选和删除；删除确认后历史与历史目录消失，但同书的书架记录、本地原文件和共享阅读数据保持不变。
9. 在启动自动目录更新期间删除一条历史，确认迟到刷新结果不会把它重新创建。
