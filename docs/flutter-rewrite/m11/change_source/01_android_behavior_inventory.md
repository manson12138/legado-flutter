...
|---|---|---|
| `BookInfoScreen` → `BookInfoIntent.ChangeSourceClick` | 从详情页打开整书换源 | 已入架网络书详情主操作“替换书源”进入独立页面并重新搜索 |
| `ReadBookScreen` → `ReadBookSheet.ChangeBookSource` | 阅读中保存进度并换源 | 阅读器顶部快捷图标与更多菜单“整书换源” |
...
13. Flutter 详情页对已入架网络书不复用进入详情前的候选列表，整书换源页每次重新读取启用书源并搜索。
14. 阅读历史中的未入架网络书不能执行整书换源，应在阅读器内提示先加入书架；单章换源不受此限制。
