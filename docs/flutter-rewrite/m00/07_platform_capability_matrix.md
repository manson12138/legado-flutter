...
| Local book file import | `ImportBookScreen`, file associations, `LocalBook` | System picker followed immediately by app-private copy; Android TXT `ACTION_VIEW` association now stages a bounded cache copy and opens the existing confirmation route | Document Picker current-readable result followed immediately by sandbox copy; external-open association pending | Android cold/hot external TXT handling and cleanup code awaits device validation; no temporary absolute path is persisted as book identity, and iOS security-scoped provider lifetime still needs validation. | Codex + user | IN_PROGRESS | 2026-07-31 |
