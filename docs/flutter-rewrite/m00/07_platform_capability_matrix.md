...
| Local book file import | `ImportBookScreen`, file associations, `LocalBook` | System picker followed immediately by app-private copy; Android TXT `ACTION_VIEW` association stages a bounded cache copy and opens the existing confirmation route | Document Picker plus app-private copy; `public.plain-text` document association receives cold/hot Scene URL contexts, coordinates security-scoped access, stages a bounded cache copy and opens the same confirmation route | Both platforms' external TXT handling and cleanup code awaits device validation; no temporary absolute path is persisted as book identity. iOS raw-text Share Extension and other external book formats remain outside this implementation. | Codex + user | IN_PROGRESS | 2026-08-02 |
...
| Android Intent associations | Association activities | URL schemes/document types/share extension | TXT document type and Scene URL receiving are implemented pending device verification; raw-text Share Extension and other formats remain deferred. | Codex + user | IN_PROGRESS | 2026-08-02 |
