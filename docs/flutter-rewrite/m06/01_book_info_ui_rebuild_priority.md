...
> 文档状态：`IN_PROGRESS / P0～P3 基础入口及封面替换、离线正文清理、重新搜索换源已写入，等待用户运行验证`  
> 创建日期：2026-07-16  
> 最后静态核对：2026-08-02  
...
- 后续批次已经接入共用封面候选搜索、本地图片复制、恢复默认封面、离线正文清理和重新搜索整书换源；
- 阅读记录主操作仍只显示后续提示，相关书、书源登录/变量、Web 文件/压缩包、日志快捷入口和跨设备同步
  仍只保留能力映射，尚未接入真实业务；
- `book_info_screen.dart` 的“后续能力”面板原有“换封面待接入”旧说明已经按真实详情菜单、主操作卡、
  封面长按和 Route 调用链校准。
...
| ViewModel | `lib/src/ui/book_info/book_info_view_model.dart` | 已覆盖详情/目录、加入书架、分组、备注、允许更新、当前候选切换和整书换源入口。 |
| Route | `lib/src/ui/book_info/book_info_route.dart` | 已处理提示、返回、分享/剪贴板、阅读器跳转、封面面板、整书换源结果、清离线正文和同名冲突对话框。 |
| Screen | `lib/src/ui/book_info/book_info_screen.dart` | 已有封面头部、四个主操作卡、简介与目录摘要、完整目录正倒序、更多菜单和底部阅读主按钮。 |
...
|---|---|---|---|
| 封面主视觉 | 封面、模糊背景、渐变遮罩、可选跟随封面色 | 已有大封面头部、标签和统一页面背景 | 未迁移 Android 模糊背景、渐变遮罩和跟随封面动态色；属于视觉差异。 |
| 顶栏 | 透明/玻璃顶栏、滚动后变色、编辑/分享/更多 | AppBar 已接入刷新、分享、封面、复制、备注、分组、允许更新、换源、清离线和删除等菜单 | 缺少透明/玻璃滚动视觉，以及书籍完整编辑页。 |
| 主操作区 | 书架、目录、书源、阅读记录四个操作卡 | 已有书架、目录、书源、阅读记录四卡，并增加封面、分组和更新目录操作 | 阅读记录仍是占位，其余真实动作待用户验证。 |
| 信息展示 | 标签、分组、分类、阅读进度、最新章节、备注、简介 | 已显示标签、分组、最新章节、阅读进度、备注和简介 | Android 部分高级字段和动态视觉仍未对齐。 |
| 目录入口 | 独立 TocActivity，详情页只展示概要 | 详情页目录摘要 + 完整目录 Sheet，支持正倒序并保持原始索引 | 尚未做独立目录路由；长目录和大字号待验证。 |
| 换源 | Sheet 支持替换、另存、冲突替换和迁移选项 | 当前搜索组换源 + M11 整书换源入口 | 缺少详情页内 Sheet 体验和迁移选项 UI。 |
| 封面 | 点击预览，长按/Sheet 换封面，保存封面 | 已接入预览、书源候选搜索、本地图片、恢复默认和历史快照同步 | 主体代码已完成，等待双端图片选择、缓存和重启恢复验证。 |
| 分组 | 长按书架操作进入分组选择，支持新增/编辑分组 | 详情页已支持选择、清除和新建分组 | 编辑/重命名现有分组仍由书架管理后续补齐。 |
| 阅读记录 | 阅读时长和时间线 Sheet | 已有独立阅读历史双页和详情占位入口 | 缺少 Android 详情页粒度的阅读时长/时间线 Sheet；不能把历史书籍列表视为已对齐。 |
| Web 文件 | 展示可下载/导入文件、压缩包条目、外部打开 | 无入口 | 需要文件下载/导入能力后实现。 |
| 菜单 | 编辑、分享、上传、同步、刷新、登录、置顶、变量、复制 URL、允许更新、拆分长章、删除提醒、清缓存、日志 | 已接入刷新、分享、封面、复制 URL、备注、分组、允许更新、整书换源、清离线正文和删除 | 完整编辑、上传/同步、书源变量、置顶、拆分长章、删除提醒和详情日志仍未映射。 |
...
|---|---|---|---|
| 封面预览和换封面 | `CoverClick`、`CoverLongClick`、`ChangeCoverSheet`、`PhotoPreview` | 已接入预览、长按、菜单和主操作卡；共用封面面板支持书源候选与本地图片 | 代码已写入，等待双端真机。 |
