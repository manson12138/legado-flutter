...
新增或修改任何持久化数据前，AI 必须先按数据语义明确选择存储，不能因为调用方便把所有状态继续写入
SQLite `caches` 表，也不能因为 MMKV 读取快就把关系数据或大内容迁入 MMKV：

1. MMKV 只用于体积小、独立、高频读取、不需要关系查询和事务的键值偏好，例如阅读显示配置、布局模式、
   搜索偏好和版本化一次性提示。
2. SQLite 用于书籍、目录、书架、历史、进度、书签、标注、替换规则、下载任务、队列、条件查询、
   用户作用域业务记录、事务状态以及大正文或带批量清理语义的缓存。
3. Token、Refresh Token、密码、密钥和其他认证凭据必须使用 Keychain/Keystore 支持的安全存储；
   Cookie 和 Authorization 继续使用既有安全边界，禁止写入普通 MMKV。
4. 大正文、图片、页面位图、Widget、Controller、Stream、Timer、平台对象和网络令牌禁止写入 MMKV。
5. 每个新增 MMKV 数据必须定义：
   - 稳定且带命名空间的 key；
   - 设备级或用户级作用域；
   - 默认值、非法值和损坏值处理；
   ...
   - 删除、失效、账号切换和版本升级生命周期；
   - 隐私边界和日志禁止内容。
6. MMKV 插件类型不得进入 Widget、ViewModel、UseCase 或领域模型；业务只能依赖项目内类型化 Store 或
   偏好接口，由组合根注入具体实现。
7. MMKV 初始化必须单飞，并遵守首帧性能规则；初始化失败必须有受控默认值或兼容存储降级，不能让 App
   永久停留在启动页。
8. 选择 SQLite 且改变 Schema 时继续执行 Schema 版本、基础建表、`onUpgrade` 和 build number 联动规则；
   选择 MMKV 时虽然不升级数据库 Schema，也必须记录 key 版本和迁移策略。
